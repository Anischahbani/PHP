...
{
    session_destroy();
    header('location:/PHP/BASES/TP/admin');
    exit();
}
...
if ( isset($_SESSION['user']) ){
    ...
    $mestables=$base->query("SHOW TABLES WHERE Tables_in_tp NOT IN (".$tables_a_exclure.")");
    while ( $matable = $mestables->fetch(PDO::FETCH_ASSOC))
    {
        echo '<a href="?table='.$matable['Tables_in_tp'].'">'.ucfirst($matable['Tables_in_tp']).'</a> ';
    }
    echo '<a href="/PHP/BASES/TP" target="_blank">Voir le front</a> ';
...
if ( !empty($_GET['table']) )
{
    ...
    <input type="hidden" class="form-control" name="table" value="'.$table_courante.'">';
    ...
    for( $i=0; $i < $nbcolonnes; $i++)
    {
        ...
        if ( $i == 0 ) { 
            ...
            $formulaire .='<input type="hidden" class="form-control" name="indice" value="'.(
        ...
        if ($i != 0 ){
            ...
            {
                $formulaire .='<textarea name="'.$info_colonne['name'].'" cols="40" rows="5" class="form-control">'.( $ligne_courante[$info_colonne['name']] ?? '' ).'</textarea></p>';
            }
            else
            {
            $formulaire .='<input type="text" size="100" id="'.$info_colonne['name'].'" name="'.$info_colonne['name'].'" class="form-control" value="'.( $ligne_courante[$info_colonne['name']] ?? '' ).'"></p>';
            }
        }
    }
    $affichage .='<th colspan="2">Actions</th>';
    $affichage .='</tr>';
    $formulaire .='<input type="submit" name="update" class="form-control" value="Valider">
                  </form> </div>';
